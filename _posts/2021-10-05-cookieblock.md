...
* Download datasets, [crawler](https://github.com/dibollinger/CookieBlock-Consent-Crawler), [classifier](https://github.com/dibollinger/CookieBlock-Consent-Classifier), and [violation detection scripts](https://github.com/dibollinger/CookieBlock-Violation-Detection)
...
Browser cookies are the most common method for tracking the session state of websites, prior works show 80-90% of websites use cookies for user tracking. The EU government have attempted to address this through regulations mandating consent, namely by General Data Protection Regulation (*GDPR*) and the ePrivacy Directive.

Prior works shown that despite the consent requirement, less than half of websites show cookie banner and those that show the consent still violate even basic rules. Majority of websites do not adhere to opt-in requirement of the consent, >10% stores affirmative consent prior any user action or store positive consent despite user rejected cookies. Finally, more than half of the consent banners are designed such that they nudge users to accept all cookies.
...
*Number of websites that show the respective type of violation. The first six are novel, and have not been explored in prior work.*

We support the legal claims by referring to relevant sections of GPDR and ePrivacy Directive and [Planet49 case](https://europeanlawblog.eu/2019/10/08/planet49-cjeu-judgment-brings-some-cookie-consent-certainty-to-planet-online-tracking/) ruled by EU Court of Justice.
...
![Cookiepedia performance](https://karelkubicek.github.io/assets/images/cookieblock_paper/cookiepedia_perf.png)![Our XGBoost model performance](https://karelkubicek.github.io/assets/images/cookieblock_paper/xgboost_perf.png)![Confusion matrices](https://karelkubicek.github.io/assets/images/cookieblock_paper/conf_matrices.png)
...
*CookieBlock interface consists of simple popup and settings.*
...
* Regulatory authorities can inspect the website compliance.
...
**A:** No, all classification and enforcement happens on client-side. We also do **not** collect any scientific data and we adhere to GDPR. If you anyway ant to provide us feedback, please fill the [feedback form](https://forms.gle/tL21ruvPZq2q218P8) or leave review in the extension store, or send us an email. If you observe malfunctioning website, send us an email to <cookie.block.extension@gmail.com>, fill an [issue on GitHub](https://github.com/dibollinger/CookieBlock/issues), or make a pull-request with updated [`known_cookies.json`](https://github.com/dibollinger/CookieBlock/blob/main/src/ext_data/known_cookies.json).


**Q:** CookieBlock breaks website xyz.com. What should I do?
**A:** Easiest solution is to `Add this site as an exception` in the CookieBlock popup. If the website does not immediately start working, you might need to clear cookies for the website. If even this does not help, you can `Pause cookie removal`. No matter what technique you use, let us know about your issues by an email to <cookie.block.extension@gmail.com> or an [issue on GitHub](https://github.com/dibollinger/CookieBlock/issues).


**Q:** Does CookieBlock work outside of EU?
**A:** Yes, despite other countries does not have as advanced privacy regulations as EU, the classification model works independently of your location. For websites in other languages than English, the model can have slightly reduced performance, as it won't be able to extract as meaningful features about the cookie content. However, the most useful features for the classification are language agnostic, so this might have only a minor impact.
...
**A:** While some of the violation detection methods can produce false positives, given the vast non-compliance, it is likely that your website does violate some of the consent requirements. We also appreciate your effort given your website at least use a CMP which gives user choices, which is better than 96% of the Internet. You can contact the CMP or the web developer to inspect what can be improved. Anyway, you can contact us by <cookie.block.extension@gmail.com> and we can figure out the removal from the dataset.


**Q:** How severe are the detected violations?
**A:** While we refer to the violations as *potential*, we do so as only a judicial ruling can provide the legal certainty as to whether they are true legal violations. All of the violations imply that user consents to incorrect information, and it requires individual inspection to declare them as neglect or malice. However, severe violations as marking Google Analytics cookies as necessary or unclassified, or even omitting to declare them forces users to accept them. In such case, the website might do even worse than website without a cookie banner, as it gives user false impression of choice.


### Updates

*September 21:* The initial version of this page.
*October 5:* TBA (deadline for the camera-ready paper version)
